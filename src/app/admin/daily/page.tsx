...
const SAMPLE_STAFF: StaffWithWish[] = [
  { staffId:'s1', fullName:'田中 一郎', wish:'morning', prevSlot: null },
  { staffId:'s2', fullName:'鈴木 花子', wish:'morning', prevSlot: null },
  { staffId:'s3', fullName:'佐藤 次郎', wish:'evening', prevSlot: null },
  { staffId:'s4', fullName:'山田 三子', wish:'night',   prevSlot:'night' }, // 夜勤明け
  { staffId:'s5', fullName:'中村 四郎', wish:'night',   prevSlot: null },
  { staffId:'s6', fullName:'小林 五子', wish:'off',     prevSlot: null },
  { staffId:'s7', fullName:'加藤 六郎', wish:'evening', prevSlot: null },
]

export default function DailyAdjustPage() {
  const [currentDate, setCurrentDate] = useState(new Date(2025, 5, 12))
  ...
  // プール（未配置スタッフ）
  const [pool, setPool] = useState<StaffWithWish[]>(SAMPLE_STAFF)
  ...
  const changeDate = (dir: 1 | -1) => {
    ...
    setCurrentDate(next)
    // TODO: 日付変更時にSupabaseから該当日のデータを再取得
    setAssignment({ date: format(next, 'yyyy-MM-dd'), morning:[], evening:[], night:[] })
    setPool(SAMPLE_STAFF)
  }
            ...
            <div className="text-xs text-gray-400 space-y-1 pt-2">
              ...
              {(['morning','evening','night','off'] as const).map(w => (
                <div key={w} className="flex items-center gap-1.5">
                  ...
                  />
                  <span>{w === 'morning' ? '朝希望' : w === 'evening' ? '夕希望' : w === 'night' ? '夜希望' : '休希望'}</span>
                </div>
              ))}
            </div>
          </div>
